...
PRO Island_Evol,path=path,tstart=tstart,tstop=tstop,method=method,delay=delay, $
  ...
  IF NOT KEYWORD_SET(method) THEN method='bicube_fft'
  ...
  FOR i=tstart,tstop DO BEGIN
    ...
    IF s_res.exists THEN BEGIN
      ...
      IF KEYWORD_SET(pressure) THEN BEGIN
        ...
        IF KEYWORD_SET(ps) THEN device,/col,bits=8,file='p_con.'+ts+'.ps'
      ...
      ENDIF ELSE BEGIN
        IF KEYWORD_SET(ps) THEN device,/col,bits=8,file='poincare.'+ts+'.ps'
