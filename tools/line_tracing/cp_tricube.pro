PRO cp_tricube, apar=apar
  ; Convert apar to closed periodic form for cubic interpolation
  ...
  FOR ix=0,nx-1 DO BEGIN
    ...
    fft_1=FFT(REFORM(apar[ix,1,*]))
    fft_n=FFT(REFORM(apar[ix,ny-1,*]))
  ...
  ; For third-order finite differences, we duplicate the correct columns
  apar_cp=FLTARR(nx,ny+3,nz+3)
  FOR ix=0,nx-1 DO BEGIN
    FOR jy=0,ny+2 DO BEGIN
      apar_cp[ix,jy,*]=[apar_yp[ix,jy,nz-1],REFORM(apar_yp[ix,jy,*]), $
                        apar_yp[ix,jy,0],apar_yp[ix,jy,1]]
    ENDFOR
  ENDFOR
