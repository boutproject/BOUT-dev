			   ...
			   void *machEnv)
{
  ...
  ewt = N_VNew(neq, (machEnvType)machEnv);
  if (ewt == NULL) return(FALSE);
  acor = N_VNew(neq, (machEnvType)machEnv);
  if (acor == NULL) {
    N_VFree(ewt);
    return(FALSE);
  }
  tempv = N_VNew(neq, (machEnvType)machEnv);
  if (tempv == NULL) {
    N_VFree(ewt);
    N_VFree(acor);
    return(FALSE);
  }
  ftemp = N_VNew(neq, (machEnvType)machEnv);
  if (ftemp == NULL) {
    N_VFree(tempv);
    N_VFree(ewt);
    N_VFree(acor);
  ...
  for (j=0; j <= maxord; j++) {
    zn[j] = N_VNew(neq, (machEnvType)machEnv);
    if (zn[j] == NULL) {
      N_VFree(ewt);
      N_VFree(acor);
      N_VFree(tempv);
