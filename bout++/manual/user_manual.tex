...
See the \texttt{elm\_reduced} manual for an explanation of the output from the
...
is given in the file \texttt{mhd.cpp} in the BOUT++ examples directory. The equations to be solved are:
\begin{eqnarray*}
...
a shorthand macro. In this case, we could use this shorthand for \code{v} and \code{B}:
\begin{lstlisting}
SOLVE_FOR(v);
SOLVE_FOR(B);
\end{lstlisting}
...
This scheme is not used in \texttt{mhd.cpp}, partly for clarity, and partly because currently
...
The physics code \texttt{2fluid.cpp} implements a set of reduced Braginskii 2-fluid equations,
...
The file \texttt{highbeta\_reduced.cpp} solves for a reduced single-fluid MHD similar to that
derived in \cite{hazeltine-2003} (flute-reduced, page 282), used for linear ELM modelling.
A separate document \texttt{elm\_reduced.pdf} in this manual directory derives and explains
the model and coordinate system used.

\subsection{Compressible Fluid}

The model in \texttt{gas\_compress.cpp} solves the compressible gas dynamics equations for
the density $n$, velocity $\mathbf{V}$, and pressure $P$:

\subsection{MHD}

The file \texttt{mhd.cpp} solves the full MHD equations for the full values (perturbation + initial),
whilst the file \texttt{mhd\_perturb.cpp} solves for a perturbation about the equilibrium.
