...
# Handle the REQUIRES and CONFLICTS arguments for models, examples, and tests.
# Returns from those functions if REQUIRES are not met, or if CONFLICTS are true
...
#
# Arguments: - MODEL: Name of the executable - SOURCES: List of source files to
# compile - REQUIRES: list of variables that must be true to build model (note:
# use `CONFLICTS` to negate the variable, rather than `NOT VARIABLE`) -
...
function(bout_add_model MODEL)
  ...
  bout_handle_requires_conflicts(
    "model" MODEL REQUIRES ${BOUT_MODEL_OPTIONS_REQUIRES} CONFLICTS
    ${BOUT_MODEL_OPTIONS_CONFLICTS})
...
#
# If called from a standalone project, just builds the example as a normal
# model. If called when building the BOUT++ library itself, also copy input
# files and optionally other files, like grid files, to the library build
# directory.
#
# Arguments: - EXAMPENAME: Name of the executable - SOURCES: List of source
# files to compile - DATA_DIRS: List of data directories to copy (default:
# 'data') - EXTRA_FILES: List of other files to copy - REQUIRES: list of
# variables that must be true to build example (note: use `CONFLICTS` to negate
# the variable, rather than `NOT VARIABLE`) - CONFLICTS: list of variables that
# must be false to enable test
function(bout_add_example EXAMPLENAME)
  ...
  bout_handle_requires_conflicts(
    "example" ${EXAMPLENAME} REQUIRES ${BOUT_EXAMPLE_OPTIONS_REQUIRES}
    CONFLICTS ${BOUT_EXAMPLE_OPTIONS_CONFLICTS})

  bout_add_model(${EXAMPLENAME} SOURCES ${BOUT_EXAMPLE_OPTIONS_SOURCES})

  # If this is a standalone project, we can stop here. Otherwise, we need to
  # copy the various input files to the build directory
...
# Add a new integrated or MMS test. By default, the executable is named like the
# first source, stripped of its file extension. If no sources are given, then
# you probably at least want to set USE_RUNTEST
#
# Required arguments:
#
# * BUILD_CHECK_TARGET: the specific build-check target that should depend on
#   this test
#
...
#
# * USE_RUNTEST: if given, the test uses `./runtest` as the test command,
#   otherwise it uses the executable
#
# * USE_DATA_BOUT_INP: if given, copy `data/BOUT.inp`
#
# * EXTRA_FILES: any extra files that are required to run the test
#
# * REQUIRES: list of variables that must be true to enable test (note: use
#   `CONFLICTS` to negate the variable, rather than `NOT VARIABLE`)
#
# * CONFLICTS: list of variables that must be false to enable test
#
# * EXECUTABLE_NAME: name of the executable, if different from the first source
#   name
#
...
function(bout_add_integrated_or_mms_test BUILD_CHECK_TARGET TESTNAME)
  ...
  set(multiValueArgs SOURCES EXTRA_FILES REQUIRES CONFLICTS TESTARGS
                     EXTRA_DEPENDS)
  cmake_parse_arguments(BOUT_TEST_OPTIONS "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})

  bout_handle_requires_conflicts(
    "test" ${TESTNAME} REQUIRES ${BOUT_TEST_OPTIONS_REQUIRES} CONFLICTS
    ${BOUT_TEST_OPTIONS_CONFLICTS})

  if(BOUT_TEST_OPTIONS_SOURCES)
    # We've got some sources, so compile them into an executable and link
    # against BOUT++
    ...
    # Set the name of the executable. We either take it as an option, or use the
    # first source file, stripping the file suffix
    if(BOUT_TEST_OPTIONS_EXECUTABLE_NAME)
      ...
      get_filename_component(BOUT_TEST_EXECUTABLE_NAME
                             ${BOUT_TEST_FIRST_SOURCE} NAME_WE)
      set_target_properties(${TESTNAME} PROPERTIES OUTPUT_NAME
                                                   ${BOUT_TEST_EXECUTABLE_NAME})
    endif()
  ...
  if(BOUT_TEST_OPTIONS_DOWNLOAD)
    ...
    add_custom_target(download_test_data
                      DEPENDS ${BOUT_TEST_OPTIONS_DOWNLOAD_NAME})
...
function(bout_add_integrated_test TESTNAME)
  bout_add_integrated_or_mms_test(build-check-integrated-tests ${TESTNAME}
                                  ${ARGV})
endfunction()
...
# Add an alias for an imported target Workaround for CMAke < 3.18 Taken from
# https://github.com/conan-io/conan/issues/2125#issuecomment-351176653
...
# Call nx-config with an argument, and append the resulting path to a list Taken
# from
# https://github.com/LiamBindle/geos-chem/blob/feature/CMake/CMakeScripts/FindNetCDF.cmake
