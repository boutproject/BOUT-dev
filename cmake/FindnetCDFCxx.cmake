...
#
# This module uses the ``ncxx4-config`` helper script as a hint for the location
# of the NetCDF C++ library. It should be in your PATH.
#
# This module will define the following variables:
#
# ::
#
# netCDFCxx_FOUND - true if netCDFCxx was found netCDFCxx_VERSION - netCDFCxx
# version in format Major.Minor.Release netCDFCxx_INCLUDE_DIRS - Location of the
# netCDFCxx includes netCDFCxx_LIBRARIES - Required libraries
#
# This module will also export the ``netCDF::netcdf-cxx4`` target.
#
# You can also set the following variables:
#
# ``netCDFCxx_ROOT`` Specify the path to the netCDF C++ installation to use
#
# ``netCDFCxx_DEBUG`` Set to TRUE to get extra debugging output
...
find_program(
  NCXX4_CONFIG "ncxx4-config"
  PATHS "${netCDFCxx_ROOT}"
  PATH_SUFFIXES bin
  DOC "Path to netCDF C++ config helper"
  NO_DEFAULT_PATH)

find_program(NCXX4_CONFIG "ncxx4-config" DOC "Path to netCDF C++ config helper")
...
bout_inspect_netcdf_config(NCXX4_HINTS_INCLUDE_DIR "${NCXX4_CONFIG}"
                           "--includedir")
...
find_path(
  ...
  HINTS "${netCDF_C_INCLUDE_DIR}" "${NCXX4_HINTS_INCLUDE_DIR}"
        "${NCXX4_HINTS_PREFIX}" "${NCXX4_CONFIG_LOCATION}"
...
find_library(
  ...
  HINTS "${NCXX4_HINTS_INCLUDE_DIR}" "${NCXX4_HINTS_PREFIX}"
...
if(_ncxx4_version)
  ...
  string(REGEX REPLACE "netcdf-cxx4 \([0-9]+\\.[0-9]+\\.[0-9]+\).*" "\\1"
                       netCDFCxx_VERSION "${_ncxx4_version_lower}")
...
if(netCDFCxx_FOUND)
  ...
  if(NOT TARGET netCDF::netcdf-cxx4)
    ...
    set_target_properties(
      netCDF::netcdf-cxx4
      PROPERTIES IMPORTED_LINK_INTERFACE_LIBRARIES netCDF::netcdf
