...
function(petsc_get_version)
  if(EXISTS "${PETSC_DIR}/include/petscversion.h")
    ...
    foreach(line ${vstrings})
      ...
      )# break line into three fields (the first is always "#define")
...
"
  ...
  file(STRINGS "${PETSC_INCLUDE_CONF}/petscconf.h" PETSC_HAS_OPENMP
       REGEX "#define PETSC_HAVE_OPENMP 1"
  )
  if(PETSC_HAS_OPENMP)
  ...
  petsc_test_compiles(
    "${petsc_includes_minimal};${petsc_mpi_include_dirs}"
    "${PETSC_LIBRARIES_TS};${petsc_additional_libraries}" petsc_works_minimal
  )
  if(petsc_works_minimal)
  ...
  else(petsc_works_minimal
  )# Minimal includes fail, see if just adding full includes fixes it
    petsc_test_compiles(
      "${petsc_includes_all};${petsc_mpi_include_dirs}"
    ...
    if(petsc_works_allincludes) # It does, we just need all the includes (
    ...
    else(petsc_works_allincludes
    )# We are going to need to link the external libs explicitly
      ...
      if(petsc_works_alllibraries)
        ...
        petsc_test_compiles(
          "${petsc_includes_all};${petsc_mpi_include_dirs}"
          "${PETSC_LIBRARIES_TS};${petsc_additional_libraries}" petsc_works_all
        )
...
if(NOT PETSC_INCLUDES AND NOT TARGET PETSc::PETSc)
  ...
  if(PkgConfig_FOUND)
    ...
    set(PETSC_LIBRARIES
        ${PkgPETSC_LINK_LIBRARIES}
        CACHE STRING "PETSc libraries" FORCE
    )
    set(PETSC_INCLUDES
        ${PkgPETSC_INCLUDE_DIRS}
        CACHE STRING "PETSc include path" FORCE
    )
    set(PETSC_EXECUTABLE_COMPILES
        "YES"
...
mark_as_advanced(
  PETSC_INCLUDES PETSC_LIBRARIES PETSC_COMPILER PETSC_DEFINITIONS PETSC_MPIEXEC
  PETSC_EXECUTABLE_COMPILES
)
...
if(PETSC_FOUND)
  if(NOT TARGET PETSc::PETSc)
    ...
    set_target_properties(
      PETSc::PETSc PROPERTIES IMPORTED_LOCATION "${PETSC_LIBRARY}"
