...
LaplaceXZcyclic::LaplaceXZcyclic(Mesh *m, Options *options, const CELL_LOC loc) : LaplaceXZ(m, options, loc), mesh(m) {
  ...
  k1d = Array<dcomplex>(nmode);
  k1d_2 = Array<dcomplex>(nmode);
...
Field3D LaplaceXZcyclic::solve(const Field3D &rhs, const Field3D &x0) {
  ...
  for(int y=mesh->ystart; y <= mesh->yend; y++) {
    ...
    if(mesh->firstX()) {
      ...
      if(inner_boundary_flags & INVERT_SET) {
        ...
        rfft(&x0(mesh->xstart - 1, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d));
        rfft(&x0(mesh->xstart, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d_2));
        for(int kz = 0; kz < nmode; kz++) {
        ...
        rfft(&rhs(mesh->xstart - 1, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d));
        rfft(&rhs(mesh->xstart, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d_2));
        for(int kz = 0; kz < nmode; kz++) {
          // Use the same coefficients as applied to the solution
          // so can either set gradient or value
    ...
    for(int x=mesh->xstart; x <= mesh->xend; x++) {
      ...
      rfft(&rhs(x, y, mesh->zstart), mesh->zend + 1 - mesh->zstart, std::begin(k1d));
      for(int kz = 0; kz < nmode; kz++) {
    ...
    if(mesh->lastX()) {
      ...
      if(outer_boundary_flags & INVERT_SET) {
        ...
        rfft(&x0(mesh->xend, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d));
        rfft(&x0(mesh->xend + 1, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d_2));
        for(int kz = 0; kz < nmode; kz++) {
          // Use the same coefficients as applied to the solution
          // so can either set gradient or value
        ...
        // Fourier transform rhs in Z at xstart-1 and xstart
        rfft(&rhs(mesh->xend, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d));
        rfft(&rhs(mesh->xend + 1, y, mesh->zstart), mesh->zend + 1 - mesh->zstart,
             std::begin(k1d_2));
        for(int kz = 0; kz < nmode; kz++) {
          // Use the same coefficients as applied to the solution
          // so can either set gradient or value
  ...
  for(int y=mesh->ystart; y <= mesh->yend; y++) {
    for(int x=xstart;x<=xend;x++) {
      ...
      irfft(std::begin(k1d), mesh->zend + 1 - mesh->zstart, &result(x, y, mesh->zstart));
    }
    ind += nmode;
  }
