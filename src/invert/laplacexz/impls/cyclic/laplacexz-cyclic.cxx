...
LaplaceXZcyclic::LaplaceXZcyclic(Mesh *m, Options *options) : LaplaceXZ(m, options), mesh(m) {
  ...
  k1d = new dcomplex[(mesh->local_nz)/2 + 1];
  k1d_2 = new dcomplex[(mesh->local_nz)/2 + 1];
...
Field3D LaplaceXZcyclic::solve(const Field3D &rhs, const Field3D &x0) {
  ...
  for(int y=mesh->ystart; y <= mesh->yend; y++) {
    ...
    if(mesh->firstX()) {
      ...
      if(inner_boundary_flags & INVERT_SET) {
        ...
        rfft(&x0(mesh->xstart-1,y,0), mesh->local_nz, k1d);
        rfft(&x0(mesh->xstart,y,0), mesh->local_nz, k1d_2);
        for(int kz = 0; kz < nmode; kz++) {
        ...
        rfft(&rhs(mesh->xstart-1,y,0), mesh->local_nz, k1d);
        rfft(&rhs(mesh->xstart,y,0), mesh->local_nz, k1d_2);
        for(int kz = 0; kz < nmode; kz++) {
          // Use the same coefficients as applied to the solution
          // so can either set gradient or value
          rhscmplx[ind + kz][0] = bcoef[ind + kz][0]*k1d[kz] + ccoef[ind + kz][0]*k1d_2[kz];
    ...
    for(int x=mesh->xstart; x <= mesh->xend; x++) {
      ...
      rfft(&rhs(x,y,0), mesh->local_nz, k1d);
      for(int kz = 0; kz < nmode; kz++) {
    ...
    if(mesh->lastX()) {
      ...
      if(outer_boundary_flags & INVERT_SET) {
        ...
        rfft(&x0(mesh->xend,y,0), mesh->local_nz, k1d);
        rfft(&x0(mesh->xend+1,y,0), mesh->local_nz, k1d_2);
        for(int kz = 0; kz < nmode; kz++) {
          // Use the same coefficients as applied to the solution
          // so can either set gradient or value
        ...
        // Fourier transform rhs in Z at xstart-1 and xstart
        rfft(&rhs(mesh->xend,y,0), mesh->local_nz, k1d);
        rfft(&rhs(mesh->xend+1,y,0), mesh->local_nz, k1d_2);
        for(int kz = 0; kz < nmode; kz++) {
          // Use the same coefficients as applied to the solution
          // so can either set gradient or value
          rhscmplx[ind + kz][nloc-1] = acoef[ind + kz][nloc-1]*k1d[kz] + bcoef[ind + kz][nloc-1]*k1d_2[kz];
  ...
  for(int y=mesh->ystart; y <= mesh->yend; y++) {
    for(int x=xstart;x<=xend;x++) {
      ...
      irfft(k1d, mesh->local_nz, &result(x,y,0));
    }
    ind += nmode;
  }
