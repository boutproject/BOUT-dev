...
LaplaceXY2::LaplaceXY2(Mesh* m, Options* opt, const CELL_LOC loc)
    : localmesh(m), indexConverter(std::make_shared<GlobalIndexer<Field2D>>(
  ...
  if (direct) {
    ...
    // , atol ) where r_k = b - Ax_k. The solution is considered diverged if |r_k| > dtol
    // * |b|.
...
void LaplaceXY2::setCoefs(const Field2D& A, const Field2D& B) {
  ...
  for (const auto& index : indexConverter->getRegionNobndry()) {
