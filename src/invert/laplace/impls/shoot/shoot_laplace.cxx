...
const FieldPerp LaplaceShoot::solve(const FieldPerp &rhs) {
  ...
  if(mesh->lastX()) {
    ...
    for(int ix=xe;ix<mesh->local_nx;ix++)
  ...
  for(int ix=xe; ix >= xs; ix--) {
    rfft(rhs[ix], mesh->local_nz, rhsk);
