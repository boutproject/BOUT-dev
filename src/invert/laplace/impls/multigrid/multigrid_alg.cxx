...
int nn = i * mm + k;
BoutReal val =
    b[nn] - matmg[level][nn * 9 + 3] * x[nn - 1] - matmg[level][nn * 9 + 5] * x[nn + 1]
    - matmg[level][nn * 9 + 1] * x[nn - mm] - matmg[level][nn * 9 + 7] * x[nn + mm]
    - matmg[level][nn * 9] * x[nn - mm - 1] - matmg[level][nn * 9 + 2] * x[nn - mm + 1]
...
// Receive from z-
rtag = zProcM;
ierr = bout::globals::mpi->MPI_Irecv(&x[lnz[level] + 2], 1, xvector, zProcM, rtag, commMG,
                                     &requests[2]);
ASSERT1(ierr == MPI_SUCCESS);
...
// Send to z-
stag = rProcI + numP;
ierr = bout::globals::mpi->MPI_Isend(&x[lnz[level] + 3], 1, xvector, zProcM, stag, commMG,
                                     &requests[1]);
ASSERT1(ierr == MPI_SUCCESS);
...
// Receive from x-
rtag = xProcM;
ierr = bout::globals::mpi->MPI_Irecv(&x[0], lnz[level] + 2, MPI_DOUBLE, xProcM, rtag,
                                     commMG, &requests[2]);
ASSERT1(ierr == MPI_SUCCESS);
}
...
;
ierr =
    bout::globals::mpi->MPI_Irecv(&x[(lnx[level] + 1) * (lnz[level] + 2)], lnz[level] + 2,
                                  MPI_DOUBLE, xProcP, rtag, commMG, &requests[3]);
ASSERT1(ierr == MPI_SUCCESS);

// Send to x+
stag = rProcI;
ierr = bout::globals::mpi->MPI_Isend(&x[lnx[level] * (lnz[level] + 2)], lnz[level] + 2,
