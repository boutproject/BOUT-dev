...
LaplaceMultigrid::LaplaceMultigrid(Options *opt)
  ...
  if (mglevel > 1) {
    ...
    for (int n = aclevel; n > 1; n--) {
      if (nn % 2 != 0) {
        output << "Size of local x-domain is not a power of 2^" << mglevel
    ...
    for (int n = aclevel; n > 1; n--) {
      if (nn % 2 != 0) {
        output << "Size of local z-domain is not a power of 2^ " << aclevel
               << " mglevel is changed to " << aclevel - n + 1 << endl;
        aclevel = aclevel - n + 1;
        n = 1;
      }
      nn = nn / 2;
...
#pragma omp for
    for (int k = 1; k < llz + 1; k++) {
      ...
      BoutReal ddz =
          D(i2, yindex, k2) * coords->g33(i2, yindex) / coords->dz / coords->dz;
      // coefficient of 2nd derivative stencil (z-direction)

      BoutReal dxdz = D(i2, yindex, k2) * coords->g13(i2, yindex) /
                      coords->dx(i2, yindex) / coords->dz / 2.;
      ...
      BoutReal dxd =
          (D(i2, yindex, k2) * 2. * coords->G1(i2, yindex) +
      ...
      BoutReal dzd = (D(i2, yindex, k2) * 2. * coords->G3(i2, yindex) +
                      ...
                      coords->g13(i2, yindex) * ddx_C // (could assume zero, at least
                                                      // initially, if easier; then check
