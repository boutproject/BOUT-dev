...
public:
  LaplacePetsc(Options *UNUSED(opt) = nullptr, const CELL_LOC UNUSED(loc) = CELL_CENTRE, Mesh *UNUSED(mesh_in) = bout::globals::mesh) {
...
class LaplacePetsc : public Laplacian {
public:
  LaplacePetsc(Options *opt = nullptr, const CELL_LOC loc = CELL_CENTRE, Mesh *mesh_in = bout::globals::mesh);
  ~LaplacePetsc() {
    KSPDestroy( &ksp );
    VecDestroy( &xs );
    VecDestroy( &bs );
