...
void LaplacePDD::start(const FieldPerp &b, PDD_data &data) {
  ...
  for (kz = 0; kz <= maxmode; kz++) {
    ...
    if (mesh->firstX()) {
      ...
      tridag(data.avec[kz], data.bvec[kz], data.cvec[kz], data.bk[kz], data.xk[kz],
             mesh->xend + 1);

      // Add C (row m-1) from next processor

      e[mesh->xend] = data.cvec[kz][mesh->xend];
      tridag(data.avec[kz], data.bvec[kz], data.cvec[kz], e, data.w[kz], mesh->xend + 1);
      ...
      tridag(data.avec[kz] + mesh->xstart, data.bvec[kz] + mesh->xstart,
             data.cvec[kz] + mesh->xstart, e, data.v[kz] + mesh->xstart, mesh->xend + 1);
