...
void LaplacePDD::start(const FieldPerp& b, PDD_data& data) {
  ...
  for (kz = 0; kz <= maxmode; kz++) {
    ...
    if (localmesh->firstX()) {
      ...
      tridag(&data.avec(kz, localmesh->xstart), &data.bvec(kz, localmesh->xstart),
             &data.cvec(kz, localmesh->xstart), std::begin(e),
             &data.v(kz, localmesh->xstart), localmesh->xend + 1);
      ...
      e[0] = data.avec(kz, localmesh->xstart);
      tridag(&data.avec(kz, localmesh->xstart), &data.bvec(kz, localmesh->xstart),
             &data.cvec(kz, localmesh->xstart), &e[localmesh->xstart],
             &data.v(kz, localmesh->xstart), localmesh->xend - localmesh->xstart + 1);
      e[0] = 0.0;
