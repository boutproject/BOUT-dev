...
void Laplacian::tridagCoefs(int jx, int jy, BoutReal kwave, dcomplex& a, dcomplex& b,
  ...
  coef3 = 2. * contravariant_components.g13(jx, jy); ///< X-Z mixed derivative coefficient
  ...
  if (c1coef != nullptr) {
    ...
    if ((jx > 0) && (jx < (localmesh->LocalNx - 1))) {
      ...
      coef4 += contravariant_components.g11(jx, jy) * dc2dx_over_c1;
      coef5 += contravariant_components.g13(jx, jy) * dc2dx_over_c1;
    }
  }

  if (localmesh->IncIntShear) {
    // d2dz2 term
    coef2 += contravariant_components.g11(jx, jy) * localcoords->IntShiftTorsion(jx, jy)
      ...
      if (kz == 0) {
        ...
        if (inner_boundary_flags & INVERT_DC_GRAD
          ...
          for (int ix = 0; ix < inbndry; ix++) {
            avec[ix] = 0.;
            bvec[ix] = 1.;
            cvec[ix] = -exp(-k * coords->dx(ix, jy) / sqrt(contravariant_components.g11(ix, jy)));
          }
      ...
      // AC i.e. kz =/= 0 (all other modes than the offset mode)
        ...
        if (inner_boundary_flags & INVERT_AC_GRAD
          ...
          for (int ix = 0; ix < inbndry; ix++) {
            avec[ix] = 0.0;
            bvec[ix] = 1.0;
            cvec[ix] = -exp(-1.0 * sqrt(contravariant_components.g33(ix, jy) / contravariant_components.g11(ix, jy)) * kwave
                            * coords->dx(ix, jy));
          }
        } else if (inner_boundary_flags & INVERT_IN_CYLINDER) {
          // Condition for inner radial boundary for cylindrical coordinates
    ...
    if (localmesh->lastX()) {
      ...
      if (kz == 0) {
        ...
        if (outer_boundary_flags & INVERT_DC_GRAD
          ...
          for (int ix = 0; ix < inbndry; ix++) {
            ...
            bvec[ncx - ix] = 1.;
            avec[ncx - ix] =
                -exp(-k * coords->dx(ncx - ix, jy) / sqrt(contravariant_components.g11(ncx - ix, jy)));
          }
        } else {
          // Order 2 dirichlet BC (boundary half between points)
      ...
      // AC i.e. kz =/= 0 (all other modes than the offset mode)
        ...
        if (outer_boundary_flags & INVERT_AC_GRAD
          ...
          for (int ix = 0; ix < outbndry; ix++) {
            avec[ncx - ix] =
                -exp(-1.0 * sqrt(contravariant_components.g33(xe - ix, jy) / contravariant_components.g11(xe - ix, jy))
                     * kwave * coords->dx(xe - ix, jy));
            bvec[ncx - ix] = 1.0;
            cvec[ncx - ix] = 0.0;
          }
        } else {
