...
PetscErrorCode laplacePCapply(PC pc, Vec x, Vec y) {
  // Get the context
  LaplaceXYpetsc* s = nullptr;
  const auto ierr = PCShellGetContext(pc, reinterpret_cast<void**>(&s));
  CHKERRQ(ierr);
...
LaplaceXYpetsc::LaplaceXYpetsc(Mesh* m, Options* opt, const CELL_LOC loc)
  ...
  if (not finite_volume) {
    ...
    if (not localmesh->include_corner_cells) {
      throw BoutException(
          "Finite difference form of LaplaceXYpetsc allows non-orthogonal x- and "
  ...
  for (RangeIterator it = localmesh->iterateBndryLowerY(); !it.isDone(); it++) {
    // Should not go into corner cells, LaplaceXYpetsc stencil does not include them
...
void LaplaceXYpetsc::setPreallocationFiniteVolume(PetscInt* d_nnz, PetscInt* o_nnz) {
  const int localN = localSize();
...
void LaplaceXYpetsc::setPreallocationFiniteDifference(PetscInt* d_nnz, PetscInt* o_nnz) {
  const int localN = localSize();
...
void LaplaceXYpetsc::setCoefs(const Field2D& A, const Field2D& B) {
  Timer timer("invert");
...
void LaplaceXYpetsc::setMatrixElementsFiniteVolume(const Field2D& A, const Field2D& B) {
  //////////////////////////////////////////////////
  // Set Matrix elements
  //
...
void LaplaceXYpetsc::setMatrixElementsFiniteDifference(const Field2D& A,
                                                       const Field2D& B) {
  //////////////////////////////////////////////////
  // Set Matrix elements
  //
...
Field2D LaplaceXYpetsc::solve(const Field2D& rhs, const Field2D& x0) {
  Timer timer("invert");
  ...
  if (reason <= 0) {
    throw BoutException("LaplaceXYpetsc failed to converge. Reason {} ({:d})",
  ...
  for (RangeIterator it = localmesh->iterateBndryLowerY(); !it.isDone(); it++) {
    if (
        // Should not go into corner cells, finite-volume LaplaceXYpetsc stencil does not include
        // them
...
void LaplaceXYpetsc::solveFiniteVolume(const Field2D& x0) {
  // Use original LaplaceXYpetsc implementation of passing boundary values for backward
  // compatibility
  if (localmesh->firstX()) {
    if (x_inner_dirichlet) {
      for (int y = localmesh->ystart; y <= localmesh->yend; y++) {
...
void LaplaceXYpetsc::solveFiniteDifference(const Field2D& x0) {
 ...
 */
int LaplaceXYpetsc::precon(Vec input, Vec result) {
...
int LaplaceXYpetsc::globalIndex(int x, int y) {
...
void LaplaceXYpetsc::savePerformance(Solver& solver, const std::string& name) {
...
int LaplaceXYpetsc::LaplaceXYMonitor::call(Solver* /*solver*/, BoutReal /*time*/,
                                           int /*iter*/, int /*nout*/) {
...
void LaplaceXYpetsc::LaplaceXYMonitor::outputVars(Options& output_options,
