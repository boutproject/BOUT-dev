...
InvertParHypre::InvertParHypre(Mesh *mi) {
  ...
  crhs = cmatrix(m->ngy, (m->local_nz)/2 + 1);
  cresult = cmatrix(m->ngy, (m->local_nz)/2 + 1);
  ...
  mat[0] = new HYPRE_StructMatrix[m->ngx * ( (m->local_nz)/2 + 1 )];
  for(int i=1;i<m->ngx;i++)
    mat[i] = mat[i-1] + (m->local_nz)/2 + 1;
