...
InvertParCR::InvertParCR(Options *opt) : InvertPar(opt), A(1.0), B(0.0), C(0.0), D(0.0), E(0.0) {
  ...
  nsys = 1 + (mesh->local_nz)/2; 

  rhs = matrix<dcomplex>(mesh->local_ny, nsys);
  
  // Find out if we are on a boundary
  int size = mesh->local_ny-4;
  SurfaceIter surf(mesh);
  for(surf.first(); !surf.isDone(); surf.next()) {
    BoutReal ts;
    int n = mesh->local_ny-4;
    if(!surf.closed(ts)) {
      // Open field line
      if(surf.firstY())
        n += 2;
...
const Field3D InvertParCR::solve(const Field3D &f) {
  ...
  for(surf.first(); !surf.isDone(); surf.next()) {
    ...
    // Number of rows
    int y0 = 0, size = mesh->local_ny-4; // If no boundaries
    if(!closed) {
      if(surf.firstY()) {
        y0 += 2;
        size += 2;
    ...
    for(int y=0;y<mesh->local_ny-4;y++)
      rfft(f(x,y+2), mesh->local_nz, rhs[y+y0]);
    ...
    for(int y=0;y<size;y++)
      irfft(rhs[y], mesh->local_nz, result(x,y+2-y0));
