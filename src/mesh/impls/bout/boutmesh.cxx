...
void BoutMesh::post_receiveZ(CommHandle& ch) {
  ...
  if (z_minus_dest != -1) {
    mpi->MPI_Irecv(std::begin(ch.zminus_recvbuff),
                   msg_len(ch.var_list.get(), 0, MXSUB, 0, MYSUB, 0, MZG),
...
comm_handle BoutMesh::sendZ(FieldGroup& g, comm_handle handle) {
  ...
  if (z_minus_dest != -1) {
    int len = pack_data(ch->var_list.get(), MXG, MXG + MXSUB, MYG, MYG + MYSUB, MZG,
  ...
  if (z_plus_dest != -1) {
    int len = pack_data(ch->var_list.get(), MXG, MXG + MXSUB, MYG, MYG + MYSUB, MZSUB,
...
int BoutMesh::wait(comm_handle handle) {
  ...
  do {
    ...
    case Z_SENT_NEGATIVE: {
      unpack_data(ch->var_list.get(), MXG, MXG + MXSUB, MYG, MYG + MYSUB, 0, MZG,
                  std::begin(ch->zminus_recvbuff));
      break;
    }
    case Z_SENT_POSITIVE: {
      unpack_data(ch->var_list.get(), MXG, MXG + MXSUB, MYG, MYG + MYSUB, MZSUB + MZG,
