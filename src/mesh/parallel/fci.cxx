...
FCITransform::FCITransform(Mesh *m) : mesh(*m), forward_map(*m, +1), backward_map(*m, -1) {
...
void FCITransform::calcYUpDown(Field3D &f) {
  ...
  interpolate(f, f.yup(), forward_map, +1);
  interpolate(f, f.ydown(), backward_map, -1);
}
 ...
 */
void FCITransform::interpolate(Field3D &f, Field3D &f_next, const FCIMap &fcimap, int dir) {
