...
void Mesh::communicate(FieldPerp &f) {
  ...
  int nout = local_nx-xend-1; // Number of x points in outer guard cell

  // Post receives for guard cell regions

  recv[0] = irecvXIn(f[0],       nin*local_nz, 0);
  recv[1] = irecvXOut(f[xend+1], nout*local_nz, 1);
  
  // Send data
  sendXIn(f[xstart], nin*local_nz, 1);
  sendXOut(f[xend-nout+1], nout*local_nz, 0);
...
int Mesh::msg_len(const vector<FieldData*> &var_list, int xge, int xlt, int yge, int ylt) {
  ...
  for(const auto& var : var_list) {
    if(var->is3D()) {
      len += (xlt - xge) * (ylt - yge) * local_nz * var->BoutRealSize();
    } else {
