...
void BndNeumann_O2::apply(Field2D &f,BoutReal t) {
  ...
  // for nonorthogonal meshes need to shift derivatives
  Field2D xshift, yshift;
  xshift = 1./mesh->g11*(mesh->g12*DDY(f) + mesh->g13*DDZ(f));
  yshift = 1./mesh->g22*(mesh->g12*DDX(f) + mesh->g23*DDZ(f));
  ...
  if(mesh->StaggerGrids && loc != CELL_CENTRE) {
    ...
    for(bndry->first(); !bndry->isDone(); bndry->next1d()) {
      ...
      // x boundaries only
        f(bndry->x,bndry->y) = f(bndry->x-bndry->bx, bndry->y-bndry->by) + delta*val - delta*xshift(bndry->x,bndry->y);
        if (bndry->width == 2){
          f(bndry->x + bndry->bx, bndry->y + bndry->by) = f(bndry->x - 2*bndry->bx, bndry->y - 2*bndry->by) + 3.0*delta*val;
        }
      }	else if(bndry->by != 0 && bndry->bx == 0) {
      // y boundaries only
	f(bndry->x,bndry->y) = f(bndry->x-bndry->bx, bndry->y-bndry->by) + delta*val - delta*yshift(bndry->x,bndry->y);
        if (bndry->width == 2){
          f(bndry->x + bndry->bx, bndry->y + bndry->by) = f(bndry->x - 2*bndry->bx, bndry->y - 2*bndry->by) + 3.0*delta*val;
        }
      } else if(bndry->by != 0 && bndry->bx != 0) {
...
void BndNeumann_O2::apply(Field3D &f,BoutReal t) {
  ...
  // Take derivatives here, needed for later
  Field3D dfdx, dfdy, dfdz;
  dfdx = DDX(f);
  dfdy = DDY(f);
  dfdz = DDZ(f);
  output << "inside neumann_o2" << endl;
  ...
  if(mesh->StaggerGrids && loc != CELL_CENTRE) {
    ...
    for(; !bndry->isDone(); bndry->next1d()) {
      ...
      // Interpolate (linear) metrics to halfway between last cell and boundary cell
      BoutReal g11shift = 0.5*(mesh->g11(bndry->x,bndry->y) + mesh->g11(bndry->x-bndry->bx,bndry->y));
      BoutReal g22shift = 0.5*(mesh->g22(bndry->x,bndry->y) + mesh->g22(bndry->x-bndry->bx,bndry->y));
      BoutReal g12shift = 0.5*(mesh->g12(bndry->x,bndry->y) + mesh->g12(bndry->x-bndry->bx,bndry->y));
      BoutReal g23shift = 0.5*(mesh->g23(bndry->x,bndry->y) + mesh->g23(bndry->x-bndry->bx,bndry->y));
      BoutReal g13shift = 0.5*(mesh->g13(bndry->x,bndry->y) + mesh->g13(bndry->x-bndry->bx,bndry->y));

      for(int zk=0;zk<mesh->ngz-1;zk++) {
        if(fg){
          val = fg->generate(xnorm,TWOPI*ynorm,TWOPI*zk/(mesh->ngz-1),t);
        }
	// Linear interpolation to get xshift and yshift halfway between grid cells on boundary layers
        BoutReal xshift = 0.5*g12shift*(dfdy(bndry->x,bndry->y,zk) + dfdy(bndry->x-bndry->bx,bndry->y,zk)) 
		+ 0.5*g13shift*(dfdz(bndry->x-bndry->bx,bndry->y,zk) + dfdz(bndry->x,bndry->y,zk));
        BoutReal yshift = 0.5*g12shift*(dfdx(bndry->x,bndry->y,zk) + dfdx(bndry->x-bndry->bx,bndry->y,zk)) 
		+ 0.5*g23shift*(dfdz(bndry->x,bndry->y,zk) + dfdz(bndry->x-bndry->bx,bndry->y,zk));

        if(bndry->bx != 0 && bndry->by == 0){
          // x boundaries only
          f(bndry->x,bndry->y,zk) = f(bndry->x-bndry->bx, bndry->y-bndry->by,zk) + delta/g11shift*(val - xshift);
          if (bndry->width == 2){
            f(bndry->x + bndry->bx, bndry->y + bndry->by,zk) = f(bndry->x - 2*bndry->bx, bndry->y - 2*bndry->by,zk) + 3.0*delta*val;
          }
        } else if(bndry->by != 0 && bndry->bx == 0) {
        // y boundaries only
	  f(bndry->x,bndry->y,zk) = f(bndry->x-bndry->bx, bndry->y-bndry->by,zk) + delta/g22shift*(val - yshift);
          if (bndry->width == 2){
            f(bndry->x + bndry->bx, bndry->y + bndry->by,zk) = f(bndry->x - 2*bndry->bx, bndry->y - 2*bndry->by,zk) + 3.0*delta*val;
          }
        } else if(bndry->by != 0 && bndry->bx != 0) {
	  f(bndry->x,bndry->y,zk) = 0.0;
	  if (bndry->width == 2){
            f(bndry->x + bndry->bx, bndry->y + bndry->by,zk) = 0.0;
          }
        }
      }
    }
