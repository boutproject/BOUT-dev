  ...
  if(mesh->StaggerGrids && loc != CELL_CENTRE) {
	...
	for(; !bndry->isDone(); bndry->next1d()) {
	  ...
	  for(int zk=0;zk<mesh->local_nz;zk++) {
	    if(fg){
	      val = fg->generate(xnorm,TWOPI*ynorm,TWOPI*zk/(mesh->local_nz), t);
	    }
...
void BoundaryNeumann::apply(Field3D &f,BoutReal t) {
  ...
  if(mesh->StaggerGrids && loc != CELL_CENTRE) {
	...
	for(; !bndry->isDone(); bndry->next1d()) {
	  ...
	  for(int zk=0;zk<mesh->local_nz;zk++) {
	    if(fg)
	      val = fg->generate(xnorm,TWOPI*ynorm,TWOPI*zk/(mesh->local_nz),t) * metric->dx(bndry->x, bndry->y);
	...
	for(; !bndry->isDone(); bndry->next1d()) {
	  ...
	  for(int zk=0;zk<mesh->local_nz;zk++) {
	    ...
	    if(fg)
	      val = fg->generate(xnorm,TWOPI*ynorm,TWOPI*zk/(mesh->local_nz),t) * metric->dx(bndry->x - bndry->bx, bndry->y);
	...
	for(; !bndry->isDone(); bndry->next1d()) {
	  ...
	  for(int zk=0;zk<mesh->local_nz;zk++) {
	    ...
	    if(fg){
	      val = fg->generate(xnorm,TWOPI*ynorm,TWOPI*zk/(mesh->local_nz),t) * metric->dy(bndry->x, bndry->y);
	    }
	...
	for(; !bndry->isDone(); bndry->next1d()) {
	  ...
	  for(int zk=0;zk<mesh->local_nz;zk++) {
	    if(fg)
	      val = fg->generate(xnorm,TWOPI*ynorm,TWOPI*zk/(mesh->local_nz),t) * metric->dy(bndry->x, bndry->y - bndry->by);
...
void BoundaryZeroLaplace::apply(Field3D &f) {
  ...
  for(bndry->first(); !bndry->isDone(); bndry->nextY()) {
    ...
    rfft(f(x-bx,y), mesh->local_nz, c0);
    rfft(f(x-2*bx,y), mesh->local_nz, c1);
...
void BoundaryDivCurl::apply(Vector3D &var) {
  ...
  jx = mesh->xend+1;
  for(jy=1;jy<mesh->local_ny-1;jy++) {
    for(jz=0;jz<ncz;jz++) {
      jzp = (jz+1) % ncz;
      jzm = (jz - 1 + ncz) % ncz;
