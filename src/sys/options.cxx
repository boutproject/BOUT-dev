...
void Options::get(const string &key, int &val, int def) {
  ...
  if (it == options.end()) {
    ...
    val = def;
    output_info << "\tOption " << sectionName << ":" << key << " = " << def << " (default)" << endl;
    return;
  }
...
void Options::get(const string &key, bool &val, bool def) {
  ...
  if (it == options.end()) {
    ...
    if (def) {
      output_info << "\tOption " << sectionName << ":" << key << " = true   (default)" << endl;
    } else {
      output_info << "\tOption " << sectionName << ":" << key << " = false  (default)" << endl;
    }
    return;
  }
