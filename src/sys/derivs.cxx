...
const Field3D DDX(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field2D DDX(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->coordinates()->DDX(f, outloc, method, region);
}

////////////// Y DERIVATIVE /////////////////

const Field3D DDY(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexDDY(f,outloc, method, region) / f.getMesh()->coordinates()->dy;
}

const Field2D DDY(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->coordinates()->DDY(f, outloc, method, region);
}

////////////// Z DERIVATIVE /////////////////

const Field3D DDZ(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexDDZ(f,outloc, method, region) / f.getMesh()->coordinates()->dz;
}

const Field2D DDZ(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return Field2D(0.0, f.getMesh());
}

const Vector3D DDZ(const Vector3D &v, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Vector2D DDZ(const Vector2D &v, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field3D D2DX2(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field2D D2DX2(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field3D D2DY2(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field2D D2DY2(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field3D D2DZ2(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD2DZ2(f, outloc, method, region) / SQ(f.getMesh()->coordinates()->dz);
}

const Field2D D2DZ2(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return Field2D(0.0, f.getMesh());
}
...
const Field3D D4DX4(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD4DX4(f, outloc, method, region) / SQ(SQ(f.getMesh()->coordinates()->dx));
}

const Field2D D4DX4(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD4DX4(f, outloc, method, region) / SQ(SQ(f.getMesh()->coordinates()->dx));
}

const Field3D D4DY4(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD4DY4(f, outloc, method, region) / SQ(SQ(f.getMesh()->coordinates()->dy));
}

const Field2D D4DY4(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD4DY4(f, outloc, method, region) / SQ(SQ(f.getMesh()->coordinates()->dy));
}

const Field3D D4DZ4(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD4DZ4(f, outloc, method, region) / SQ(SQ(f.getMesh()->coordinates()->dz));
}

const Field2D D4DZ4(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexD4DZ4(f, outloc, method, region) / SQ(SQ(f.getMesh()->coordinates()->dz));
}
 ...
 */
const Field2D D2DXDY(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
 ...
 * ** Applies Neumann boundary in Y, communicates
 */
const Field3D D2DXDY(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  Field3D dfdy = DDY(f, outloc, method, region);
  f.getMesh()->communicate(dfdy);
  return DDX(dfdy, outloc, method, region);
}

const Field2D D2DXDZ(const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  // TODO: mesh & loc
  return Field2D(0.0);
}

/// X-Z mixed derivative
const Field3D D2DXDZ(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field2D D2DYDZ(const Field2D &UNUSED(f), CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  // TODO: mesh & loc
  return Field2D(0.0);
}

const Field3D D2DYDZ(const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
...
const Field2D VDDX(const Field2D &v, const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexVDDX(v, f, outloc, method, region) / f.getMesh()->coordinates()->dx;
}

/// General version for 2 or 3-D objects
const Field3D VDDX(const Field3D &v, const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexVDDX(v, f, outloc, method, region) / f.getMesh()->coordinates()->dx;
}
...
const Field2D VDDY(const Field2D &v, const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexVDDY(v, f, outloc, method, region) / f.getMesh()->coordinates()->dy;
}

// general case
const Field3D VDDY(const Field3D &v, const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexVDDY(v, f, outloc, method, region) / f.getMesh()->coordinates()->dy;
}
...
// special case where both are 2D
const Field2D VDDZ(const Field2D &UNUSED(v), const Field2D &UNUSED(f), CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  // TODO: loc, mesh
  return Field2D(0.0);
}

// Note that this is zero because no compression is included
const Field2D VDDZ(const Field3D &UNUSED(v), const Field2D &UNUSED(f), CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  // TODO: loc, mesh
  return Field2D(0.0);
}

// general case
const Field3D VDDZ(const Field3D &v, const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
 ...
 *******************************************************************************/
const Field2D FDDX(const Field2D &v, const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexFDDX(v, f, outloc, method, region) / f.getMesh()->coordinates()->dx;
}

const Field3D FDDX(const Field3D &v, const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexFDDX(v, f, outloc, method, region) / f.getMesh()->coordinates()->dx;
}

/////////////////////////////////////////////////////////////////////////

const Field2D FDDY(const Field2D &v, const Field2D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexFDDY(v, f, outloc, method, region) / f.getMesh()->coordinates()->dy;
}

const Field3D FDDY(const Field3D &v, const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
  return f.getMesh()->indexFDDY(v, f, outloc, method, region) / f.getMesh()->coordinates()->dy;
}
...
const Field2D FDDZ(const Field2D &v, const Field2D &UNUSED(f), CELL_LOC UNUSED(outloc), REGION UNUSED(region), DIFF_METHOD UNUSED(method)) {
  // TODO: mesh & loc
  return Field2D(0.0);
}

const Field3D FDDZ(const Field3D &v, const Field3D &f, CELL_LOC outloc, REGION region, DIFF_METHOD method) {
