...
bool Ncxx4::openw(const char *name, bool append) {
  ...
  if(append) {
    ...
    if((xDim.getSize() != mesh->local_nx) || (yDim.getSize() != mesh->local_ny) || (zDim.getSize() != mesh->local_nz)
       || (!tDim.isUnlimited()) ) {
      delete dataFile;
      dataFile = NULL;
      return false;
    ...
    xDim = dataFile->addDim("x", mesh->local_nx);
    if(xDim.isNull()) {
      delete dataFile;
      dataFile = NULL;
      return false;
    ...
    yDim = dataFile->addDim("y", mesh->local_ny);
    if(yDim.isNull()) {
      delete dataFile;
      dataFile = NULL;
      return false;
    ...
    zDim = dataFile->addDim("z", mesh->local_nz);
    if(zDim.isNull()) {
      delete dataFile;
      dataFile = NULL;
      return false;
