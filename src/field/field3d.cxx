...
void Field3D::applyBoundary(const string &condition) {
  ...
  for(const auto& reg : fieldmesh->getBoundaries()) {
    BoundaryOp* op = bfact->create(condition, reg.get());
    op->apply(*this);
    delete op;
  }
  ...
  if (background != nullptr) {
    ...
    *this = tot - (*background);
    ...
    for(const auto& reg : fieldmesh->getBoundariesPar()) {
      BoundaryOpPar* op = bfact->create(condition, reg.get());
      op->apply(*this);
      delete op;
    }
  }
    ...
    *this = tot - (*background);
    ...
    for(const auto& reg : fieldmesh->getBoundariesPar()) {
      if(reg->label.compare(region) == 0) {
        ...
        BoundaryOpPar* tmp = bfact->create(condition, reg.get());
        // then clone that with the actual argument
        BoundaryOpPar* op = tmp->clone(reg.get(), f);
        op->apply(*this);
        delete tmp;
        delete op;
        break;
