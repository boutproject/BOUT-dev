...
Field3D::Field3D(Mesh *msh) : background(nullptr), fieldmesh(msh), deriv(nullptr), yup_field(nullptr), ydown_field(nullptr) {
  ...
  if(fieldmesh) {
    nx = fieldmesh->local_nx;
    ny = fieldmesh->local_ny;
    nz = fieldmesh->local_nz;
  }
#ifdef CHECK
  else {
    nx=-1;
...
void checkData(const Field3D &f)  {
  ...
  for(auto d : f) {
    if( (d.x < mesh->xstart) or (d.x > mesh->xend) or (d.y < mesh->ystart) or (d.y > mesh->yend) or (d.z >= mesh->local_nz))
