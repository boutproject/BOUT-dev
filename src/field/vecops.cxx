...
const Vector2D V_dot_Grad(const Vector2D &v, const Vector2D &a)
{
  ...
  if(a.covariant) {
    /* Copy-paste-search-replace template for covariant
     *  result.j = VDDX(vcn.x, a.j) + VDDY(vcn.y, a.j) + VDDZ(vcn.z, a.j);
     *  result.j -= vcn.x*(mesh->G1_j1*a.x + mesh->G2_j1*a.y + mesh->G3_j1*a.z);
    ...
    /* Copy-paste-search-replace template for contravariant
     *   result.j = VDDX(vcn.x, a.j) + VDDY(vcn.y, a.j) + VDDZ(vcn.z, a.j);
     *   result.j += vcn.x*(mesh->Gj_11*a.x + mesh->Gj_21*a.y + mesh->Gj_31*a.z);
...
const Vector3D b0xGrad_dot_Grad(const Field3D &phi, const Vector3D &A, CELL_LOC outloc) {
...
#ifdef CHECK
        int msg_pos = msg_stack.push("b0xGrad_dot_Grad( Field3D , Vector3D )");
#endif
...
#pragma omp section
        ...
        result = (rx + ry + rz) / (mesh->J*sqrt(mesh->g_22));
