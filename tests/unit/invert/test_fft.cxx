...
TEST(FFTTest, DST) {
  ...
  // Get numbers 0 to size-1
  Array<BoutReal> indices(size);
  std::iota(indices.begin(), indices.end(), 0);

  // Calculate sin(x) on [0, 2pi]
  Array<BoutReal> input(size);
  std::transform(indices.begin(), indices.end(), input.begin(),
                 [](BoutReal i) -> BoutReal { return std::sin(i * TWOPI / size); });

  Array<dcomplex> output(size);

  // Compute forward DST
  DST(input.begin(), size, output.begin());
...
TEST(FFTTest, DST_rev) {
  ...
  // Make a spectrum with a single frequency
  Array<dcomplex> input(size);
  std::fill(input.begin(), input.end(), dcomplex{0., 0.});
  input[1] = dcomplex{0., -0.5};
  ...
  // Compute inverse DST
  DST_rev(input.begin(), size, output.begin());
