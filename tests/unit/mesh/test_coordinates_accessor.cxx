...
namespace {
struct TestCoordinates : public Coordinates {
  using Coordinates::Coordinates;
  using Coordinates::splitBxyParallelSlices;
};
} // namespace
...
TEST_F(CoordinatesAccessorTest, ClearBoth) {
  ...
  if (bout::build::use_metric_3d) {
    coords.splitBxyParallelSlices();
  }

  const CoordinatesAccessor acc(mesh->getCoordinates());
  const CoordinatesAccessor acc2(&coords); // Different from previous Coordinates
