...
if full_test:
    # requires 3d refinement, so only test if full_test==true
...
results += cycle_staggering2('', 'y', ngrids, testfunc, testfunc2, boutcore.Vpar_Grad_par, Vpar_Grad_par, order, types_2d) # no staggering allowed
...
if test_deriv_ops:
    ...
    results += cycle_staggering2('', 'y', ngrids, testfunc, testfunc2, boutcore.VDDY, lambda v,f: v*DDY(f), order, types_2d)
