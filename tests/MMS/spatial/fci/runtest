...
from numpy import array, linspace, log, polyfit
...
def make_grid(nz: int):
    ...
    maps = zb.make_maps(grid, field, nslice=1, quiet=True, MXG=1)
    zb.write_maps(
        grid,
        field,
        maps,
        ...
        quiet=True,
        gridfile=f"mesh.{nz}.fci.nc",
    )


def run_fci_operators(nslice: int, nz: int, name: str) -> dict[str, float]:

    # Command to run
    args = f"MYG={nslice} {name} mesh:file=mesh.{nz}.fci.nc"
    cmd = f"./fci_mms {args}"
...
def check_fci_operators(name: str, case: dict) -> bool:
    ...
    for n in NLIST:
        errors = run_fci_operators(nslice, n, args)
