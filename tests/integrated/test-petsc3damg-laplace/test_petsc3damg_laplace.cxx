...
int main(int argc, char** argv) {
  ...
  b1 = d1*Laplace_perp(f1) + Grad_perp(c1)*Grad_perp(f1)/c1 + a1*f1;
  sol1 = 0.;
  ...
  try {
    ...
    checkData(sol1);
    bcheck1 = d1*Laplace_perp(sol1) + Grad_perp(c1)*Grad_perp(sol1)/c1 + a1*sol1;
    absolute_error1 = f1-sol1;
    max_error1 = max(abs(absolute_error1, RGN_NOBNDRY), true);
  ...
  b2 = d2*Laplace_perp(f2) + Grad_perp(c2)*Grad_perp(f2)/c2 + a2*f2;
  sol2 = 0.;
  ...
  try {
    ...
    bcheck2 = d2*Laplace_perp(sol2) + Grad_perp(c2)*Grad_perp(sol2)/c2 + a2*sol2;
    absolute_error2 = f2-sol2;
    max_error2 = max(abs(absolute_error2, RGN_NOBNDRY), true);
  } catch (BoutException &err) {
  ...
  b3 = d3*Laplace_perp(f3) + Grad_perp(c3)*Grad_perp(f3)/c3 + a3*f3;
  sol3 = 0.;
  ...
  if (mesh->firstX()) {
    for (int j=0;j<mesh->LocalNy;j++) {
      for (int k=0;k<mesh->LocalNz;k++) {
        x0(mesh->xstart-1,j,k) = 0.5*(f3(mesh->xstart-1,j,k)+f3(mesh->xstart,j,k));
      }
    }
  }
  if (mesh->lastX()) {
    for (int j=0;j<mesh->LocalNy;j++) {
      for (int k=0;k<mesh->LocalNz;k++) {
        x0(mesh->xend+1,j,k) = 0.5*(f3(mesh->xend+1,j,k)+f3(mesh->xend,j,k));
      }
    }
  }
  ...
  try {
    ...
    bcheck3 = d3*Laplace_perp(sol3) + Grad_perp(c3)*Grad_perp(f3)/c3 + a3*sol3;
    absolute_error3 = f3-sol3;
    max_error3 = max(abs(absolute_error3, RGN_NOBNDRY), true);
  } catch (BoutException &err) {
  ...
  b4 = d4*Laplace_perp(f4) + Grad_perp(c4)*Grad_perp(f4)/c4 + a4*f4;
  sol4 = 0.;
  ...
  try {
    ...
    bcheck4 = d4*Laplace_perp(sol4) + Grad_perp(c4)*Grad_perp(sol4)/c4 + a4*sol4;
    absolute_error4 = f4-sol4;
    max_error4 = max(abs(absolute_error4, RGN_NOBNDRY), true);
  } catch (BoutException &err) {
