...
Field3D DDY_weights(const Field3D& f) {
  ...
  for (int i = mesh->xstart; i <= mesh->xend; i++) {
    for (int j = mesh->ystart; j <= mesh->yend; j++) {
      for (int k = 0; k < mesh->LocalNz; k++) {
        std::vector<ParallelTransform::PositionsAndWeights> pw_up =
            pt.getWeightsForYUpApproximation(i, j, k);
        std::vector<ParallelTransform::PositionsAndWeights> pw_down =
            pt.getWeightsForYDownApproximation(i, j, k);
