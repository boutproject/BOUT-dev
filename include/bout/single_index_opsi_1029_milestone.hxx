...
//  auto izmxm = izm.xm();
   ...
   auto ixp = i_xp(i,ny,nz);
   auto ixm = i_xm(i,ny,nz);
   auto izp = i_zp(i,nz);
   auto izm = i_zm(i,nz);

   auto izpxp = i_xp(izp,ny,nz);
   auto izpxm = i_xm(izp,ny,nz);
   auto izmxp = i_xp(izm,ny,nz);
   auto izmxm = i_xm(izm,ny,nz);
...
// use raw pointer to field data 
  ...
  const auto iyp = i_yp(i,nz); // auto iyp = i.yp();
  const auto iym = i_ym(i,nz); // auto iym = i.ym():
  const int  ind_2d = i / nz;  // index for Field2D data (has no z-dependence)
  const int  iyp_2d = iyp/ nz;
  const int  iym_2d = iym/nz;
