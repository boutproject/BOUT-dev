...
private:
  ...
  const FieldMetric& I_m;

public:

  TokamakCoordinatesFactory(Mesh& mesh, const Field2D& Rxy, const Field2D& Bpxy, const Field2D& Btxy, const FieldMetric& B, const FieldMetric& hthe, const FieldMetric& I)
      : mesh_m(mesh), Rxy_m(Rxy), Bpxy_m(Bpxy), Btxy_m(Btxy), B_m(B), hthe_m(hthe), I_m(I) {}

  Coordinates* make_tokamak_coordinates(const BoutReal& sign_of_bp = 1.0) const
  {
  ...
  const FieldMetric& I() const { return I_m; }
