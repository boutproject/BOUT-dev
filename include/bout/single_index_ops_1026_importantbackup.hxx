...
RAJA_HOST_DEVICE BoutReal Div_par_Grad_par_g(const FieldAccessor<location> &f, const SpecificInd<N> &i) {
 ...
 // Use the raw pointers to yup/ydown fields by t_yup and t_down. They both use 3D index;
  ...
  BoutReal gradient_upper = 2.*(pmn_yup[iyp.ind] - f[i]) / (dy + t_dy[iyp_2d]); // metric->dy[i] is t_dy
  BoutReal flux_upper = gradient_upper * (J + t_J[iyp_2d]) / (g_22 + t_g22[iyp_2d]); //metric->g22[i]
  
  BoutReal gradient_lower = 2.*(f[i] - pmn_ydown[iym.ind]) / (dy + t_dy[iyp_2d]);
  BoutReal flux_lower = gradient_lower * (J + t_J[iym_2d]) / (g_22 + t_g22[iym_2d]);
