...
/////  Baesed on Ben Duddson, Steven Glenn code, Yining Qin update from 0521-2020
...
public:
  ...
  int rhs(BoutReal UNUSED(time)) {
   ...
   // set field region index for GPU
    auto indices = n.getRegion("RGN_NOBNDRY").getIndices(); //set index of region
    Ind3D *ob_i = &(indices[0]);

///*
//  RAJA GPU code ----------- start

    RAJA::forall<EXEC_POL>(RAJA::RangeSegment(0, n.getRegion("RGN_NOBNDRY").getIndices().size()), [=] RAJA_DEVICE (int id) {	

		auto i = ob_i[id]; // get Ind3D index
		BoutReal div_current = alpha * Div_par_Grad_par_g(phi_minus_n_acc, i);		
		DDT(n_acc)[id] =  - bracket_g(phi_acc, n_acc, i)
                	    - div_current
		 ...
		 DDT(vort_acc)[id] = - bracket_g(phi_acc, vort_acc, i)
			      - div_current
