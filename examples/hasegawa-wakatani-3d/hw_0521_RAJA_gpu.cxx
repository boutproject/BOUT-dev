...
BoutReal gradient_upper = 2.*(f[kyp] - f[k+1]) / (dy[k]+dy[kyp]);
...
BoutReal gradient_lower = 2.0*(f[k]-f[-kym-1])/(dy[k]+ dy[kyp]);
...
//return (flux_upper - flux_lower)/(dy[k]*J[i]);
return output ;
//return 0.0;
}
...
// variables need for RHS evaluation
  ...
  BoutReal* phi_minus_n_yup= nullptr;
...
inline void BOUT_DEVICE calcTimeDerivatives(const int i) {
   ...
   BoutReal div_current = alpha * Div_par_Grad_par_G(i, p_phi_minus_n,  p_g22, dxMesh,dyMesh, dzMesh,JMesh,nxMesh, nyMesh, nzMesh);
...
//std::unique_ptr<Laplacian> phiSolver; // Laplacian solver for vort -> phi
  ...
  int rhs(BoutReal UNUSED(time)) {
   ...
   // phi_minus_n_yup = phi_minus_n.fast_coords.yup();
