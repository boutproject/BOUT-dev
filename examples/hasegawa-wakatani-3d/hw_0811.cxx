...
public:
  Field3D n, vort,cp_n;  // Evolving density and vorticity
  Field3D phi, cp_phi_minus_n;      // Electrostatic potential
...
// variables need for RHS evaluation
 ...
 FieldAccessor<>* cp_phi_minus_n_acc;

  std::vector<Ind3D, std::allocator<Ind3D>> o_i; 
...
inline void RAJA_DEVICE test0(const int i ){

//	BoutReal d=gpu_Div_par_Grad_par(cp_phi_minus_n_acc, o_i[i]);
//	return i;
	BoutReal d =0;
}
...
auto cp_phi_minus_n_acc = FieldAccessor<>(cp_phi_minus_n);
 
RAJA::forall<EXEC_POL>(RAJA::RangeSegment(0, o_i.size()), [=] RAJA_DEVICE (int index) {
	test0(index);	
...
std::cout << typeid(phi_minus_n_acc).name() << '\n';
//	cp_phi_minus_n_acc = FieldAccessor<>(cp_phi_minus_n);
 test1(); 
