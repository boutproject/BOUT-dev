...
//----prepare varaibles for GPU--------------------------------------------- end
  ...
  //------------------ Copy the data members used by GPU device -------------- start
    ...
    //Ind3D  ob_i[100]; // A std::vector of Ind3D objects
   //SpecificInd<N>
  //------------------ Copy the data members used by GPU device -------------- end
	...
	auto indices = region.getIndices();   // A std::vector of Ind3D objects
	 Ind3D * ob_i = new  Ind3D[indices.size()];
	for (int i =0;i< indices.size();i++){	ob_i[i] = indices[i];}
//	ob_i[0] = indices[0];
	//std::cout << typeid(ob_i).name() << endl;

/*
	RAJA::forall<EXEC_POL>(RAJA::RangeSegment(0, indices.size()), [=] RAJA_DEVICE (int index) {
		 auto i = ob_i[index];
          auto x = gpu_Div_par_Grad_par(phi_minus_n_acc, i);	
	});
*/


RAJA::forall<EXEC_POL>(RAJA::RangeSegment(0, indices.size()), [=] BOUT_DEVICE (int i) {
	
	auto index = ob_i[i];
	auto x = gpu_Div_par_Grad_par(phi_minus_n_acc, index); 
	auto y = gpu_DZZ_par(phi_acc,index);
