...
int main(int argc, char** argv) {
  ...
  try {
    sol2 = invert->solve(b2.slice(mesh->ystart));
  ...
  try {
    sol3 = invert->solve(b3.slice(mesh->ystart), x0.slice(mesh->ystart));
  ...
  if (mesh->firstX())
    for (int k=0;k<mesh->ngz-1;k++)
      x0(mesh->xstart-1,mesh->ystart,k) = (f4(mesh->xstart,mesh->ystart,k)-f4(mesh->xstart-1,mesh->ystart,k))
                                        /mesh->dx(mesh->xstart,mesh->ystart)
                                        /sqrt(mesh->g_11(mesh->xstart,mesh->ystart));
  if (mesh->lastX())
    for (int k=0;k<mesh->ngz-1;k++)
      x0(mesh->xend+1,mesh->ystart,k) = (f4(mesh->xend+1,mesh->ystart,k)-f4(mesh->xend,mesh->ystart,k))
                                        /mesh->dx(mesh->xend,mesh->ystart)
                                        /sqrt(mesh->g_11(mesh->xend,mesh->ystart));

  try {
    sol4 = invert->solve(b4.slice(mesh->ystart), x0.slice(mesh->ystart));
...
Field3D this_Grad_perp2(const Field3D &f) {
  Field3D result = mesh->G1 * ::DDX(f) +  mesh->G3 * ::DDZ(f) +
                   mesh->g11 * ::D2DX2(f) + mesh->g33 * ::D2DZ2(f) +
...
Field3D this_Grad_perp_dot_Grad_perp(const Field3D &f, const Field3D &g) {
  Field3D result = mesh->g11 * ::DDX(f) * ::DDX(g) + mesh->g33 * ::DDZ(f) * ::DDZ(g)
                   + mesh->g13 * (DDX(f)*DDZ(g) + DDZ(f)*DDX(g));
