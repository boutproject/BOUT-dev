...
import numpy as np
from pythonRoutines.common import obtainGeometry, getChristoffel
...
#{{{convergenceTest
def convergenceTest(paths, extension='.png', showPlot=False):
    ...
    for path in paths:
        ...
        # Get the geometry
        y_YX, x_YX, zero, alpha =\
                obtainGeometry(path, xguards=False, yguards=False)
        # Get the Christoffel symbols
        C = getChristoffel(alpha, zero)

        for coeff in C.keys():
            C[coeff]['bout'] =\
                    collect(coeff, xguards=False, yguards=False,\
                    path=path, info=False)
            # Pick the zeroth z-point (the Field2D quantities are
            # isotropic in BOUT++)
            ...
            # Create the entries in the dictionary if not already done
            if not(coeff in CAll.keys()):
                CAll[coeff] = {'error_2':[], 'error_inf':[], 'spacing':[]}
            # The error in the 2-norm and infintiy-norm
            try:
                CAll[coeff]['error_2']  .append(np.sqrt(np.mean(C[coeff]['relErr']**2.0)))
                CAll[coeff]['error_inf'].append( np.max(np.abs(C[coeff]['relErr'])) )
            except FloatingPointError:
                CAll[coeff]['error_2'] .append(0)
                CAll[coeff]['error_inf'].append(0)

            # Store the spacing in the CAll[coeff]
            CAll[coeff]['spacing'].append(np.max([dx_spacing, dy_spacing, dz_spacing]))

    for coeff in C.keys():
        print("\n\nNow processing {}".format(coeff))
        # Sort the data
        CAll[coeff] = sort_data(CAll[coeff])

        # Find the order of convergence in the 2 norm and infinity norm
        order_2, order_inf = getOrder(CAll[coeff])

        # Get the root name of the path (used for saving files)
        rootFolder = paths[0].split('/')[0] + '/'

        # Remove the root folder and put in front
        name = 'convergence'
        name = os.path.join(rootFolder, name)
        # Create folder if not exists
        if not os.path.exists(name):
            os.makedirs(name)
        name = os.path.join(name, coeff)

        # Print the convergence rate
        printConvergenceRate(CAll[coeff], order_2, order_inf, name)

        # Plot
        # We want to show the lines of the last orders, so we send in
        # order_2[-1] and order_inf[-1]
        figNo += 1
        makePlot(figNo, coeff, CAll[coeff], order_2[-1], order_inf[-1],\
                name, extension)
...
#{{{printConvergenceRate
def printConvergenceRate(data, order_2, order_inf, name):
...
#{{{makePlot
def makePlot(figNo, coeff, data, order_2, order_inf, name,\
            extension):
    """Function which handles the actual plotting"""

    # Try to make a figure with the current backend
    try:
        fig = plt.figure(figNo, figsize = plt_size)
    except:
    ...
    # Set title
    ax.set_title(r"$"+coeff[0]+"^"+coeff[1:3]+"{"+coeff[3:]+"}$")
