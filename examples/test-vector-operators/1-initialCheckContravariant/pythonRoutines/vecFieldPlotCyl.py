...
def plotVecField(path,\
    ...
    # NOTE: We have specified that myVec has contravariant components,
    # so the rho component of myvec will be myVecx, in the covariant
    # formalism there would've been a underscore between myVec and rho
    myVecx = collect('myVecx', xguards=False, yguards=False, path=path, info=False)
    myVecy = collect('myVecy', xguards=False, yguards=False, path=path, info=False)
    myVecz = collect('myVecz', xguards=False, yguards=False, path=path, info=False)
    #}}}

    #{{{Slice the data in time and add theta slice
    vecs = [myVecx, myVecy, myVecz]
    ...
    vecX = vecRhoRT*(X_TR/np.sqrt(X_TR**2 + Y_TR**2)) - vecThetaRT*Y_TR
    vecY = vecRhoRT*(Y_TR/np.sqrt(X_TR**2 + Y_TR**2)) + vecThetaRT*X_TR
