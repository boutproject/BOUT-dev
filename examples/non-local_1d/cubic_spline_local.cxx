...
void CubicSpline::calculate(const Field3D &pass_input) {
  if (direction=='y') {
	...
	for (int jz=0; jz<mesh->ngz-1; jz++)
// 	  for (int jy=mesh->yend+1-staggered; jy<mesh->ngy; jy++)
	  for (int jy=mesh->yend+1-upper_boundary_offset; jy<mesh->ngy; jy++)
	    input[rup.ind][jy][jz] = 0.;
	    
      grad_input = DDY(input,input.getLocation())*mesh->dy; // derivative in index space
