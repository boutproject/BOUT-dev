...
void NonLocalParallel::initialise(BoutReal pass_electron_charge, BoutReal pass_electron_mass, BoutReal pass_ion_mass, BoutReal pass_epsilon_0, BoutReal pass_logLambda, const bool pass_fluxes_location_is_ylow, BoutReal pass_gamma_factor) {
  ...
  #endif
  #ifdef BC_HEATFLUX
    heatflux_transients_factors = new BoutReal[2*(mesh->xend-mesh->xstart+1)*(mesh->local_nz)];
  #endif
  #ifdef CALCULATE_VISCOSITY
  ...
  #endif
  #ifdef BC_VISCOSITY
    viscosity_transients_factors = new BoutReal[2*(mesh->xend-mesh->xstart+1)*(mesh->local_nz)];
  #endif
  #ifdef CALCULATE_FRICTION
...
void NonLocalParallel::calculate_nonlocal_closures_cell_centre(const Field3D &n_electron, const Field3D &T_electron
  ...
  if (mesh->UpXSplitIndex()!=0 || mesh->DownXSplitIndex()!=0) throw BoutException("This code cannot currently handle x-splitting of processors.");
    if (!is_lower_boundary) {
      ...
      {
	mesh->wait(mesh->irecvYInOutdest(*pass_dimensionless_length.getData(),mesh->local_nx*(mesh->local_nz),
				       NONLOCAL_PARALLEL_TAGBASE + position->jx*mesh->local_nz+position->jz));
      }
  ...
  {
    Timer timer("comms");
    mesh->sendYOutOutdest(*increasing_dimensionless_length.slice(mesh->yend+1).getData(),mesh->local_nx*(mesh->local_nz),
			  NONLOCAL_PARALLEL_TAGBASE + position->jx*mesh->local_nz+position->jz);
  }
  ...
  y_broadcast(*total_dimensionless_length.getData(), mesh->local_nx*mesh->local_nz, mesh->getNYPE()-1);
...
//   throw BoutException("cell_centre version of boundary conditions code needs checking, at the moment it has just been cut&pasted from cell_ylow");
  ...
  if (is_lower_boundary) {
	...
	BoutReal Te_here = interp_to_point_YLOW(T_electron,*position);
	  ...
	  */
	  ...
	  heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (lower_boundary_n11 - interim_lower_boundary_n11)*W11_dot_W11
								    - sum_decayed_W11_W11_term*(upper_boundary_n11 - interim_upper_boundary_n11) )
								    / ( pow(W11_dot_W11,2) - pow(sum_decayed_W11_W11_term,2) );
	  heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (upper_boundary_n11 - interim_upper_boundary_n11)*W11_dot_W11
	  ...
	  */
	  ...
	  viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (lower_boundary_n20 - interim_lower_boundary_n20)*W20_dot_W20
								    - sum_decayed_W20_W20_term*(upper_boundary_n20 - interim_upper_boundary_n20) )
								    / ( pow(W20_dot_W20,2) - pow(sum_decayed_W20_W20_term,2) );
	  viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (upper_boundary_n20 - interim_upper_boundary_n20)*W20_dot_W20
	  ...
	  heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (upper_boundary_n11-interim_upper_boundary_n11)*( -sum_decayed_W11_W20_term*sum_decayed_W20_W11_term*sum_decayed_W20_W20_term
	  ...
	  heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (upper_boundary_n11-interim_upper_boundary_n11)*( -pow(sum_decayed_W20_W20_term,2)*W11_dot_W11
																					  + sum_decayed_W20_W11_term*sum_decayed_W20_W20_term*W11_dot_W20
																					  - sum_decayed_W11_W20_term*sum_decayed_W20_W20_term*W20_dot_W11
																					  + sum_decayed_W11_W20_term*sum_decayed_W20_W11_term*W20_dot_W20
	  ...
	  viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (upper_boundary_n11-interim_upper_boundary_n11)*( sum_decayed_W11_W20_term*pow(sum_decayed_W20_W11_term,2)
	  ...
	  viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz] = ( (upper_boundary_n11-interim_upper_boundary_n11)*( -sum_decayed_W20_W11_term*sum_decayed_W20_W20_term*W11_dot_W11
																					   + pow(sum_decayed_W20_W11_term,2)*W11_dot_W20
																					   - sum_decayed_W11_W11_term*sum_decayed_W20_W20_term*W20_dot_W11
																					   - W11_dot_W20*pow(W20_dot_W11,2)
																					   + sum_decayed_W11_W11_term*sum_decayed_W20_W11_term*W20_dot_W20
  ...
  #ifdef BC_HEATFLUX
    y_broadcast(heatflux_transients_factors, (mesh->xend-mesh->xstart+1)*(mesh->local_nz)*2, 0);
  #endif
  #ifdef BC_VISCOSITY
    y_broadcast(viscosity_transients_factors, (mesh->xend-mesh->xstart+1)*(mesh->local_nz)*2, 0);
  #endif
  ...
  for (int jx=mesh->xstart; jx<=mesh->xend; jx++)
	...
	for (int i=0; i<number_of_negative_eigenvalues; i++) {
	  #ifdef CALCULATE_HEATFLUX
	    heatflux_lower_boundary_transients[i][jx][jz] = W11_B_times_WinverseB_11[i]*heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    heatflux_upper_boundary_transients[i][jx][jz] = W11_B_times_WinverseB_11[i]*heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	  #ifdef CALCULATE_VISCOSITY
	    viscosity_lower_boundary_transients[i][jx][jz] = W20_B_times_WinverseB_11[i]*heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    viscosity_upper_boundary_transients[i][jx][jz] = -W20_B_times_WinverseB_11[i]*heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	  #ifdef CALCULATE_FRICTION
	    friction_lower_boundary_transients[i][jx][jz] = C10_1k_dot_W1k_B_times_WinverseB_11[i]*heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    friction_upper_boundary_transients[i][jx][jz] = C10_1k_dot_W1k_B_times_WinverseB_11[i]*heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	}
      #elif defined(BC_VISCOSITY) && !defined(BC_HEATFLUX)
	for (int i=0; i<number_of_negative_eigenvalues; i++) {
	  #ifdef CALCULATE_HEATFLUX
	    heatflux_lower_boundary_transients[i][jx][jz] = W11_B_times_WinverseB_20[i]*viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    heatflux_upper_boundary_transients[i][jx][jz] = -W11_B_times_WinverseB_20[i]*viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	  #ifdef CALCULATE_VISCOSITY
	    viscosity_lower_boundary_transients[i][jx][jz] = W20_B_times_WinverseB_20[i]*viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    viscosity_upper_boundary_transients[i][jx][jz] = W20_B_times_WinverseB_20[i]*viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	  #ifdef CALCULATE_FRICTION
	    friction_lower_boundary_transients[i][jx][jz] = C10_1k_dot_W1k_B_times_WinverseB_20[i]*viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    friction_upper_boundary_transients[i][jx][jz] = -C10_1k_dot_W1k_B_times_WinverseB_20[i]*viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	#endif
	}
      #elif defined(BC_HEATFLUX) && defined(BC_VISCOSITY)
	for (int i=0; i<number_of_negative_eigenvalues; i++) {
	  #ifdef CALCULATE_HEATFLUX
	    heatflux_lower_boundary_transients[i][jx][jz] = W11_B_times_WinverseB_11[i]*heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz]
							    + W11_B_times_WinverseB_20[i]*viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    heatflux_upper_boundary_transients[i][jx][jz] = W11_B_times_WinverseB_11[i]*heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz]
							    - W11_B_times_WinverseB_20[i]*viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	  #ifdef CALCULATE_VISCOSITY
	    viscosity_lower_boundary_transients[i][jx][jz] = W20_B_times_WinverseB_11[i]*heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz]
							      + W20_B_times_WinverseB_20[i]*viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    viscosity_upper_boundary_transients[i][jx][jz] = -W20_B_times_WinverseB_11[i]*heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz]
							      + W20_B_times_WinverseB_20[i]*viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	  #ifdef CALCULATE_FRICTION
	    friction_lower_boundary_transients[i][jx][jz] = C10_1k_dot_W1k_B_times_WinverseB_11[i]*heatflux_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz]
							    + C10_1k_dot_W1k_B_times_WinverseB_20[i]*viscosity_transients_factors[(jx-mesh->xstart)*(mesh->local_nz)+jz];
	    friction_upper_boundary_transients[i][jx][jz] = C10_1k_dot_W1k_B_times_WinverseB_11[i]*heatflux_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz]
							    - C10_1k_dot_W1k_B_times_WinverseB_20[i]*viscosity_transients_factors[(mesh->xend-mesh->xstart+1)*(mesh->local_nz)+(jx-mesh->xstart)*(mesh->local_nz)+jz];
	  #endif
	}
      #else
	for (int i=0; i<number_of_negative_eigenvalues; i++) {
...
void NonLocalParallel::rms_over_y(const Field3D &input_field, FieldPerp &output_field) {
  ...
  if (mesh->getYProcIndex()==0) {
    ...
    mesh->sendToProc(mesh->getXProcIndex(),mesh->getNYPE()-1,*output_field.getData(),mesh->local_nx*mesh->local_nz,NONLOCAL_PARALLEL_TAGBASE);
  }
  else if (mesh->getYProcIndex()==mesh->getNYPE()-1) {
    mesh->wait(mesh->receiveFromProc(mesh->getXProcIndex(),0,*output_field.getData(),mesh->local_nx*mesh->local_nz,NONLOCAL_PARALLEL_TAGBASE));
  }
}
