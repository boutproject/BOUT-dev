...
try:
  ...
  for i in range(0,tlength,t_interval):
    for j in range(2,ylength-2):
      ...
      q[i/t_interval][j] = -3.16*n[i,2,j,0]*Te[i,2,j,0]*tau_ei/electron_mass*gradT
  stagger = False
      
Temax = [0.]*(tlength/t_interval+1)
for i in range(0,tlength,t_interval):
  Temax[i/t_interval] = max(Te[i,2,:,0])
Timax = [0.]*(tlength/t_interval+1)
for i in range(0,tlength,t_interval):
  Timax[i/t_interval] = max(Ti[i,2,:,0])
nmax = [0.]*(tlength/t_interval+1)
for i in range(0,tlength,t_interval):
  nmax[i/t_interval] = max(n[i,2,:,0])
freestreammax = [0.]*(tlength/t_interval+1)
for i in range(tlength/t_interval+1):
  freestreammax[i]=3./2.*nmax[i]*Temax[i]*sqrt((Temax[i]+Timax[i])/ion_mass)
...
for i in range(0,tlength,t_interval):
  pyplot.figure(i)
  pyplot.plot(range(2,ylength-2),q[i/t_interval][2:-2],'k', range(2,ylength-2),[freestreammax[i/t_interval]]*(ylength-4), 'r',[-freestreammax[i/t_interval]]*(ylength-4), 'r')
