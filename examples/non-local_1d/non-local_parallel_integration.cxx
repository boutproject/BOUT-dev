...
void NonLocalParallelIntegration::calculateIntegralBelow_cell_centre(BoutReal eigenvalue, const Field3D &dimensionless_length_deltas_above, CubicSpline &cubic_spline_inverse_lambdaC, CubicSpline &cubic_spline_drive_term, const int &counter) {
  ...
  do {
	...
	mesh->wait(mesh->irecvYInIndest(&integral_below[*position],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + mesh->DownXSplitIndex()*mesh->local_nz*counter + mesh->local_nz*position->jx + position->jz));
      }
      else {
	mesh->wait(mesh->irecvYInOutdest(&integral_below[*position],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + (mesh->local_nx-mesh->DownXSplitIndex())*mesh->local_nz*counter + mesh->local_nz*(position->jx-mesh->DownXSplitIndex()) + position->jz));
      }
    }
    do {
    ...
    if (position->jx < mesh->UpXSplitIndex()) {
      Timer timer("comms");
      mesh->sendYOutIndest(&integral_below[*position],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + mesh->UpXSplitIndex()*mesh->local_nz*counter + mesh->local_nz*position->jx + position->jz);
    }
    else {
      Timer timer("comms");
      mesh->sendYOutOutdest(&integral_below[*position],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + (mesh->local_nx-mesh->UpXSplitIndex())*mesh->local_nz*counter + mesh->local_nz*(position->jx-mesh->UpXSplitIndex()) + position->jz);
    }
...
void NonLocalParallelIntegration::calculateIntegralAbove_cell_centre(BoutReal eigenvalue, const Field3D &dimensionless_length_deltas_above, CubicSpline &cubic_spline_inverse_lambdaC, CubicSpline &cubic_spline_drive_term, const int &counter) {
  ...
  do {
    ...
    if (mesh->lastY())
      ...
      if (position->jx<mesh->UpXSplitIndex()) {
	mesh->wait(mesh->irecvYOutIndest(&integral_above[position->jx][position->jyp][position->jz],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + mesh->UpXSplitIndex()*mesh->local_nz*counter + mesh->local_nz*position->jx + position->jz));
      }
      else {
	mesh->wait(mesh->irecvYOutOutdest(&integral_above[position->jx][position->jyp][position->jz],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + (mesh->local_nx - mesh->UpXSplitIndex())*mesh->local_nz*counter + mesh->local_nz*(position->jx-mesh->UpXSplitIndex()) + position->jz));
      }
    }
    do {
    ...
    if (position->jx < mesh->DownXSplitIndex()) {
      Timer timer("comms");
      mesh->sendYInIndest(&integral_above[*position],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + mesh->DownXSplitIndex()*mesh->local_nz*counter + mesh->local_nz*position->jx + position->jz);
    }
    else {
      Timer timer("comms");
      mesh->sendYInOutdest(&integral_above[*position],1,NONLOCAL_PARALLEL_INTEGRATION_TAGBASE + (mesh->local_nx - mesh->DownXSplitIndex())*mesh->local_nz*counter + mesh->local_nz*(position->jx-mesh->DownXSplitIndex()) + position->jz);
    }
