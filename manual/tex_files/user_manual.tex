...
In BOUT++, these equations can be solved in two ways. One method fourier
transforms in the $z$-direction, whilst the other is solving the full two
dimensional problem by matrix inversion (see the coordinates manual for
details). However, if $g_{xy}$ and $g_{yz}$ are non-zero, BOUT++ is currently
neglecting the $y$-parallel derivatives when using the solvers
...
(\ref{eq:full_laplace_inv}) $y$ plane by $y$ plane.
...
and solve a tridiagonal system for each mode.  These inversion problems are
...
\lstinline!Field2D!
%
 or constants\footnote{This \texttt{InvertPar} class can handle cases with
 closed field-lines and twist-shift boundary conditions for tokamak
 simulations}. In
%
...
\nabla_\perp f =& \nabla f - \mathbf{b}\left(\mathbf{b}\cdot\nabla\right) \\ =&
    \left(\frac{\partial f}{\partial x} - \frac{g_{xy}}{g_{yy}}\frac{\partial
    f}{\partial y}\right)\nabla x + \left(\frac{\partial f}{\partial z} -
    \frac{g_{yz}}{g_{yy}}\frac{\partial f}{\partial y}\right)\nabla z
\end{align*}
%
...
%
\begin{align}
\nabla_\perp^2 = \frac{1}{J}\frac{\partial}{\partial
...
%
\begin{align}
  \frac{1}{J}\left(J A g^{xx}\frac{\partial f}{\partial x}\right) +
  \frac{1}{J}\left(J A g^{zz}\frac{\partial f}{\partial z}\right) + B f = b
\end{align}
%
