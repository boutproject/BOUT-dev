...
16 & Set Dirichlet boundary condition to values in boundary guard cells of second argument, \code{x0}, of \lstinline!Laplacian::solve(const Field3D &b, const Field3D &x0)!.  Useful for setting a Dirichlet BC to values which are $\neq 0$ or $f(y)$ \\
32 & Set Dirichlet boundary condition to values in boundary guard cells of RHS, \code{b} in  \lstinline!Laplacian::solve(const Field3D &b, const Field3D &x0)!.  Useful for setting a Dirichlet BC to values which are $\neq 0$ or $f(y)$\\
