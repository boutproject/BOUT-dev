...
{\url{http://github.com/boutproject/BOUT-dev}}. You can the latest stable
version from {\url{https://github.com/boutproject/BOUT-dev/releases}}. If you
want to develop BOUT++, you should use git to clone the repository. To obtain a
copy of the latest version, run
%
\index{Github}
%
\begin{verbatim}
...
run any of the examples you'll also need NetCDF-4 or HDF5 installed.
...
The latest versions of NetCDF have separated out the C++ API from the main C
library. As a result, you will need to download and install both. Download the
latest versions of the NetCDF-C and NetCDF-4 C++ libraries from
{\url{http://www.unidata.ucar.edu/downloads/netcdf/index.jsp}}. As of January
2017, these are versions 4.4.1.1 and 4.3.0 respectively.
...
\end{verbatim}
%
You should check where NetCDF actually installed its libraries. On some systems
this will be ``\$HOME/local/lib'', but on others it may be, e.g.
...
%
BOUT++ can currently use two different file formats: NetCDF-4%
\footnote{\url{http://www.unidata.ucar.edu/software/netcdf/}}, and HDF5%
\footnote{\url{https://www.hdfgroup.org/HDF5/}} and experimental support for
parallel flavours of both. NetCDF is a widely used format and so has many more
tools for viewing and manipulating files.  In particular, the NetCDF-4 library
can produce files in either NetCDF3 ``classic'' format, which is
backwards-compatible with NetCDF libraries since 1994 (version 2.3), or in the
newer NetCDF4 format, which is based on (and compatible with) HDF5. HDF5 is
another widely used format. If you have multiple libraries installed then BOUT++
can use them simultaneously, for example reading in grid files in NetCDF format,
but writing output data in HDF5 format.
...
try to catch segmentation faults to print a useful error, that NetCDF files are
supported, but that the parallel flavour isn't.
...
\section{Makefiles and compiling BOUT++}
%
BOUT++ has its own makefile system. These can be used to
%
\begin{enumerate}
 \item Write an example or executable (see section~\ref{sec:executables})
 \item Add a feature to BOUT++ (see section~\ref{sec:modules})
\end{enumerate}
%
In all makefiles, \texttt{BOUT\_TOP} is required!

These makefiles are sufficient for most uses, but for more complicated, an executable script 
\texttt{bout-config} can be used to get the compilation flags (see section~\ref{sec:bout-config}).

\subsection{Executables example}
%
...
\subsubsection{Multiple subdirectories}

Large physics modules can have many files, and it can be helpful to organise these into
subdirectories. An example of how to do this is in \texttt{examples/make\_subdir}.
...
\label{sec:modules}
%
If you are writing a new module (or concrete implementation) to go into the BOUT++ library, 
then it is again pretty simple
%
\begin{lstlisting}[language=make,numbers=none]
BOUT_TOP = ../..
...
\subsection{bout-config script}
\label{sec:bout-config}

The \texttt{bout-config} script is in the \texttt{bin} subdirectory of the BOUT++ distribution, 
and is generated by \texttt{configure}. This script can be used to get the compilers, flags and settings
to compile BOUT++. To get a list of available options:
%
...
\end{lstlisting}
%
This script can be used in makefiles to compile BOUT++ alongside other libraries. An example is in
\texttt{examples/make-script}.
...
output files and analysing data. A summary of available IDL routines is given in
...
\end{verbatim}
%
\note{The \code{file\_read} command (and NetCDF access generally) is
...
\texttt{scipy.io.netcdf}. The DataFile class also provides allows access to HDF5
files through the same interface, using the \texttt{h5py} module.  To open a
file using DataFile:
%
\begin{lstlisting}[language=python,numbers=none]
from boututils.datafile import DataFile
...
\end{lstlisting}
%
This way the DataFile is automatically closed at the end of the \texttt{with}
block, even if there is an error in \texttt{f.read}.
%
...
a binary grid file in NetCDF or HDF5 format. Generating input grids for tokamaks
...
than one output format has been configured, by setting the top-level option {\bf
  dump\_format} to one of the recognised file extensions: `nc' for NetCDF;
`hdf5', `hdf' or `h5' for HDF5. For example to select HDF5 instead of the
default NetCDF format put
%
\begin{lstlisting}
dump_format = hdf5
\end{lstlisting}
...
%
Variables in BOUT++ are not initialised automatically, but must be explicitly given a value. For example the following code declares a \texttt{Field3D} variable then attempts to access a particular element:
\begin{lstlisting}[numbers=none]
Field3D f;    // Declare a variable
f(0,0,0) = 1.0;  // Error!
\end{lstlisting}
This results in an error because the data array to store values in 
\texttt{f} has not been allocated. Allocating data can be done in several ways:
\begin{enumerate}
\item Initialise with a value
  \begin{lstlisting}[numbers=none]
    Field3D f = 0.0; // Allocates memory, fills with zeros
    f(0,0,0) = 1.0; // ok
  \end{lstlisting}
  That this cannot be done at a global scope, since it requires
  the mesh to already exist and have a defined size.
\item Set to a scalar value
  \begin{lstlisting}[numbers=none]
    Field3D f;
  ...
  \end{lstlisting}
  To ensure that a field has a unique underlying memory array
  call the \texttt{allocate} method before writing to individual indices.
  
\item Use \texttt{allocate()} to allocate memory
  \begin{lstlisting}[numbers=none]
    Field3D f;
...
\subsection{Initialisation of time evolved variables}
...
\code{function} option for each variable:
%
\begin{lstlisting}[language=bash,numbers=none]
[p]
...
\end{lstlisting}
%
This evaluates an analytic expression to initialise the $P$ variable. 
...
field-lines are matched onto each other. To handle this, the 
\texttt{ballooning} function applies a truncated Ballooning
...
%
\end{figure}
There is an example code \texttt{test-ballooning} which compares
methods of setting initial conditions with the ballooning transform.
...
\subsection{Initalising variables with the \texttt{FieldFactory} class}
%
This class provides a way to generate a field with a specified form. For example to create a variable \texttt{var} from options we could write
\begin{lstlisting}[numbers=none]
FieldFactory f(mesh);
Field2D var = f.create2D("var");
\end{lstlisting}
This will look for an option called ``var'', and use that expression to initialise the variable \texttt{var}. This could then be set in the BOUT.inp file or on the command line.
\begin{lstlisting}[numbers=none]
var = gauss(x-0.5,0.2)*gauss(y)*sin(3*z)
\end{lstlisting}

To do this, \texttt{FieldFactory} implements a recursive descent parser to turn a string containing something
like
%
...
\url{https://bout2011.llnl.gov/pdf/talks/Chacon_bout2011.pdf}} is a
...
step and method (e.g. $\gamma = \delta t$ for backwards Euler method). For
...
The time step can be either fixed or adaptive, and defaults to the output timestep. To set a smaller
...
\end{verbatim}
Note the \texttt{-snes\_mf} flag which is passed to PETSc. This seems to be necessary in older versions of PETSc (3.4,3.5) but perhaps not in later versions.
...
are in the Jacobian, to work out which rows can be calculated simultaneously. 

\subsubsection{Brute force Jacobian}
...
\subsubsection{Coloring Jacobian}

The coloring code in IMEX-BDF2 must specify the non-zero pattern
of the system Jacobian. Several different non-zero patterns are currently implemented, which are controlled by the \texttt{coloring\_type} option:
\begin{enumerate}
  \setcounter{enumi}{-1}
\item 7-point star (for 3D fields). Each variable depends on all the variables on its grid cell and one grid cell in each direction
\item 13-point star. Each variable depends on all variables on its grid cell and two grid cells in each direction
\end{enumerate}
If this is not the case for your problem, then the solver may not converge, or may perform poorly, since the Jacobian will not be correct. This is problematic for systems involving Laplacian inversion of vorticity (for example), since these operations couple large parts of the mesh together. In these cases it may be possible to perform the inversion as a constraint as part of the time integration. See \texttt{examples/drift-wave-constraint} for an example where this is done.
...
\section{Iterating over fields}
\label{sec:fielditeration}

In BOUT++ 4.0.0, we now have the ability to use C++ range-based for-loops. This
means that it is possible to iterate over a whole field using a single loop:
%
\begin{lstlisting}
Field3D f(0.0);
for (auto i : f) {
...
\end{lstlisting}
%
It is also possible to specify regions to iterate over using this syntax:
%
\begin{lstlisting}
Field3D f(0.0);
...
\begin{itemize}
\item \code{RGN_ALL}, which is the whole mesh;
\item \code{RGN_NOBNDRY}, which skips all boundaries;
\item \code{RGN_NOX}, which skips the x boundaries
\item \code{RGN_NOY}, which skips the y boundaries
\end{itemize}
%
...
either NetCDF, HDF5 format binary files.  During startup, BOUT++ looks
...
format which describes the Grad-Shafranov equilibrium.  These
...
depends on the extension, so ``.nc'' will open NetCDF, and ``.hdf5'' or ``.h5''
an HDF5 file.
