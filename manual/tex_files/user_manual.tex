...
run any of the examples you'll also need NetCDF-4 (prior to 4.2) installed.
...
level 2 or higher (./configure --with-checks=2)
...
\item If the error is due to non-finite numbers, increase the checking level
    (./configure --with-checks=3) to perform more checking of values and
...
\texttt{scipy.io.netcdf}.  To open a file using DataFile:
%
\begin{lstlisting}[language=python,numbers=none]
from boututils import DataFile
...
either NetCDF or PDB format binary files.  During startup, BOUT++ looks in the
grid file for the following variables.  If any are not found, a warning will be
printed and the default values used.
%
\begin{itemize}
...
refers to $k = 0$ Fourier component, AC (`alternating-current') refers to $k ̸=
0$ Fourier components.  Non-Fourier solvers use AC options (and ignore DC
