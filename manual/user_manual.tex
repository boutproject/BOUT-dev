...
library, and to run any of the examples you'll also need NetCDF-4 installed. 
...
This is caused by different sizes of ints used in compiling the CVODE library. To fix, edit
...
\item \file{BOUT.restart.*.nc} are the restart files for the last time point. 
  As of June 2012 each processor saves its own state in a separate file, but this is being worked on.
...
timestep & Starting timestep & rk4, karniadakis \\
adaptive & Adapt timestep? & rk4 \\
...
(here \lstinline!ddt(u)! and \lstinline!ddt(v)!). At the end of the preconditioner the result should be in the state variables. A preconditioner which is just the identity matrix and so does nothing is:
\begin{lstlisting}[numbers=none]
int precon(BoutReal t, BoutReal gamma, BoutReal delta) {
  u = ddt(u);
  v = ddt(v);
...
int precon(BoutReal t, BoutReal gamma, BoutReal delta) {
  ...
  u = ddt(u);
  v = gamma*Grad_par(ddt(u)) + ddt(v);
\end{lstlisting}
note that since the preconditioner is linear, it doesn't depend on $u$ or $v$ and so we can overwrite their
values. In the nonlinear case we would need to create new variables to store these intermediate values in.
As in the RHS function, since we are taking a differential of \texttt{ddt(u)}, it first needs to be communicated
to exchange guard cell values.
...
\begin{lstlisting}[numbers=none]
  ...
  v = inv->solve(v);
\end{lstlisting}
which solves $v \leftarrow \left(1 - \gamma^2\partial_{||}^2\right)^{-1} v$.
