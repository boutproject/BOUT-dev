...
\begin{itemize}
  \item MPI (Message Passing Interface) and C++ compiler (e.g. \code{mpiCC} or \code{mpicxx}) {\bf [Required]}. Should work with most implementations, though mostly tested with MPICH, LAM, and OpenMPI.
...
To run the test cases, the Python libraries must first be set up. To do this, the path to
...
Running the script \texttt{test\_suite} in the \texttt{examples} subdirectory
will run through a set of tests,
...
\begin{itemize}
\item \lstinline!collect()!
\item \lstinline!pol_slice()!
\item \lstinline!gen_surface()!
\end{itemize}
