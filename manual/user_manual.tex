...
library, and to run any of the examples you'll also need NetCDF installed. 
...
NetCDF has several different interfaces: The C interface is quite consistent between versions,
and is now the default, and the C++ interface which was introduced in version 4 and then changed in
version 4.2. The C++ interface is the most tested in BOUT++, but only works with the 4.1.x versions. To use the C++
interface, Download the 4.1.3 {\bf bundled} release of NetCDF from \url{http://www.unidata.ucar.edu/downloads/netcdf/netcdf-4_1_3/} and put the \texttt{netcdf-4.1.3.tar.gz} file into your ``install'' directory. 
To use the C interface, download a recent version of NetCDF-C from \url{http://www.unidata.ucar.edu/downloads/netcdf}, 
and follow the instructions below (but with a differen netcdf file name). 

First untar the netcdf .tar.gz file and 'cd' into the resulting directory:
\begin{verbatim}
...
\end{verbatim}
By default this will use the C interface. To use the C++ interface, specify the API using
\begin{verbatim}
--with-netcdfapi=cxx
\end{verbatim}

The configure script should now finish successfully, printing a summary of the configuration:
\begin{verbatim}
